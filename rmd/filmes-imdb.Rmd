...
## Qual diretor tem a maior e a menor nota média do público? E da crítica? E considerando ambos?

Para obter estas respostas, vamos considerar apenas diretores que dirigiram pelo menos 3 filmes.

### Maior/menor nota média do público

```{r}
filmes %>%
  group_by(Director) %>%
  filter(n() >= 3) %>%
  summarise(`Nota Média Público` = mean(Rating), Filmes = paste(Title, collapse = " / ")) %>%
  arrange(desc(`Nota Média Público`)) %>%
  filter(row_number() == 1 | row_number() == n())
```

### Maior/menor nota média da crítica

```{r}
filmes %>%
  group_by(Director) %>%
  filter(n() >= 3) %>%
  summarise(`Nota Média Critica` = mean(Metascore, na.rm = TRUE), Filmes = paste(Title, collapse = " / ")) %>%
  arrange(desc(`Nota Média Critica`)) %>%
  filter(row_number() == 1 | row_number() == n())
```
...
filmes %>%
  ...
  group_by(Director) %>%
  filter(n() >= 3) %>%
  summarise(`Nota Média Geral` = mean(RatingAndMetascore, na.rm = TRUE), Filmes = paste(Title, collapse = " / ")) %>%
  arrange(desc(`Nota Média Geral`)) %>%
  filter(row_number() == 1 | row_number() == n())
```
